...
| Tela `/entrar` — login sem senha | `POST /auth/otp/solicitar` + `POST /auth/otp/verificar` | RF-40 | [`006-autenticacao-investidor`](../../../backend/features/006-autenticacao-investidor/implement.md) — RF-37 |
| Guarda de rota (`frontend/middleware.ts`) — redireciona para `/entrar` sem sessão | `GET /auth/me` (consultado pelo restante da navegação, ex. `AuthStatus` no cabeçalho) | RF-40 | [`006-autenticacao-investidor`](../../../backend/features/006-autenticacao-investidor/implement.md) — RF-38 |
| Cadastro (RF-27) autentica automaticamente | `POST /investors` agora exige `email` e devolve `Set-Cookie` de sessão na mesma resposta | RF-27 | [`006-autenticacao-investidor`](../../../backend/features/006-autenticacao-investidor/implement.md) — RF-36 |

## Nota de segurança
O frontend trata toda resposta do backend como não-confiável para fins de exibição (sempre sanitiza antes de renderizar), mas nunca é a camada que decide se uma ação é permitida — essa decisão sempre vem do backend/contrato, conforme já estabelecido em RNF-16 (ver [`../../../backend/features/002-investimento-primario/implement.md`](../../../backend/features/002-investimento-primario/implement.md)). Desde a feature `006-autenticacao-investidor`, o frontend também não é mais a camada que afirma "quem é" o investidor — nenhuma chamada de API manda mais um `investorId` explícito; a identidade vem sempre do cookie de sessão, revalidado pelo backend a cada chamada.
