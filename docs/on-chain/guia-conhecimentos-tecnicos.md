...
# Guia de Conhecimentos Técnicos para Acompanhar o Projeto
...
Este documento cobre a base **técnica** (blockchain, tokens, segurança e metodologia de engenharia), no nível necessário para acompanhar decisões sem programar. Para regulação, compliance e modelo de negócio, veja [`guia-conhecimentos-nao-tecnicos.md`](guia-conhecimentos-nao-tecnicos.md). Para quem vai efetivamente escrever os contratos em Solidity, veja [`guia-conhecimentos-solidity.md`](guia-conhecimentos-solidity.md).
...
**Por que precisa:** é literalmente o produto do pitch ("cada token é um smart contract" — slide 3). Toda a pasta `on-chain/features/*/contracts/` descreve o comportamento desses programas antes de serem escritos.
...
**Por que precisa:** o roadmap ([`roadmap.md`](roadmap.md)) só autoriza deploy em **mainnet** na Fase 4, depois de auditoria — antes disso tudo roda em **testnet (Amoy)**. Se alguém falar em "subir para mainnet" antes da auditoria estar concluída, isso deveria soar um alarme para você.
...
**Por que precisa:** é o mecanismo central que torna o projeto compatível com KYC sem violar a LGPD. Está detalhado em [`features/001-identidade-kyc/contracts/identity-registry.md`](features/001-identidade-kyc/contracts/identity-registry.md) e na decisão [ADR-0006](decisions/ADR-0006-fronteira-onchain-offchain-kyc.md). Sem esse conceito, é difícil entender por que o CPF do investidor nunca aparece na blockchain.
...
**Por que precisa:** é uma **decisão em aberto** do projeto — [`../plan.md`](../plan.md) registra que a moeda de liquidação da compra de cotas (stablecoin vs. BRL) ainda não está travada, e isso depende de uma parceria comercial de on/off-ramp que também aparece como pergunta em aberto no pitch (slide 8).
...
**Por que precisa:** é um marco obrigatório e bloqueante no roadmap (Fase 2 e pré-requisito da Fase 4 em [`roadmap.md`](roadmap.md)). Você vai precisar orçar e contratar isso — é citado no modelo de negócio como parte do "custo inicial" (SWOT, Fraquezas, slide 6).
...
**Por que precisa:** são exatamente os itens do [`security-checklist.md`](security-checklist.md) (SEC-01 a SEC-12), que é o critério de saída antes de contratar a auditoria. Você não precisa saber corrigir esses problemas, mas precisa reconhecer os nomes quando o time técnico ou o auditor reportar um finding.
...
**Por que precisa:** é a estrutura de todo o pacote em `on-chain/` que construímos. Acompanhar o projeto, na prática, significa revisar e aprovar `spec.md` de cada feature conforme ele evolui de `draft` para `approved` — esse é o seu principal ponto de controle antes que qualquer linha de código exista.
...
**Por que precisa:** cada `test-strategy.md` (ex.: [`features/001-identidade-kyc/test-strategy.md`](features/001-identidade-kyc/test-strategy.md)) define essa pirâmide para cada feature. Você não precisa escrever os testes, mas entender a lógica te ajuda a cobrar do time: "esse contrato tem teste de fuzz? tem teste de invariante?" antes de aceitar que uma feature está pronta.
...
**Por que precisa:** os 6 ADRs em [`decisions`](decisions) já tomaram decisões que afetam diretamente o pitch (ex.: trocar ERC-1400 por ERC-3643). São o lugar certo para você questionar ou validar escolhas técnicas sem precisar entender a implementação inteira.
...
3. Conceitos 15-17 (SDD/TDD/ADR) — para saber como ler e cobrar o pacote de specs.
4. Conceitos 11-14 (segurança) — para acompanhar a Fase 2 e a auditoria.
...
Para regulação, compliance e modelo de negócio, continue em [`guia-conhecimentos-nao-tecnicos.md`](guia-conhecimentos-nao-tecnicos.md). Se você for implementar os contratos, o próximo passo é [`guia-conhecimentos-solidity.md`](guia-conhecimentos-solidity.md).
