...
| Backend da plataforma | Off-chain (`../../../backend/features/001-onboarding-e-custodia`, repositório irmão `backend/`) | Orquestra o fluxo, guarda PII em banco próprio (LGPD), aciona o provedor |

## Fluxo ponta a ponta
1. Investidor preenche cadastro na plataforma (off-chain) → RF-01.
2. Backend envia documentos ao provedor de KYC (off-chain).
3. Provedor aprova e assina uma claim `KYC_APPROVED` para a carteira custodial do investidor (criada automaticamente, ver `../../../backend/features/001-onboarding-e-custodia`).
...
## Dependências
Nenhuma — esta é a feature-base da qual `002`, `003` e `004` (em `on-chain/`) e `../../../backend/features` (repositório irmão `backend/`) dependem para checagem de compliance.
