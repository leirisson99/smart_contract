...
## Contexto
O projeto adota TDD como prática central (`../00-constitution.md`): todo contrato precisa de um teste que falhe antes de ser implementado. É preciso escolher o toolchain de desenvolvimento Solidity que melhor suporta esse fluxo.
...
- Negativas / trade-offs aceitos: scripts de deploy/integração com a plataforma (off-chain) podem ainda precisar de ferramentas JS/TS (ex.: ethers.js/viem) em `../../backend/features` (repositório irmão `backend/`) — Foundry cobre o lado dos contratos, não substitui o backend da plataforma.
