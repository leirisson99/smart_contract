...
## Objetivo
Fechar 100% do [`../on-chain/security-checklist.md`](../on-chain/security-checklist.md), rodar ferramentas de análise de segurança em todos os contratos, e deixar o código pronto para ser enviado a uma auditoria externa — pré-requisito da Fase 4 do roadmap (`../on-chain/roadmap.md`).
...
- [x] Rodar análise estática (Slither) em todos os contratos; triar e resolver findings. 22 → 9 findings; 13 corrigidos no código (ver `PROGRESS.md`, `../on-chain/slither-triage.md`), 9 restantes justificados.
- [~] Rodar análise dinâmica (Mythril) nos contratos de maior risco: `PropertyToken` e `DividendDistributor`. Não executável neste ambiente Windows (instalação nativa falha, Docker local indisponível) — pendente rodar em CI/Linux ou WSL2 (ver `../on-chain/audit-package.md`).
- [~] Revisar e marcar como mitigados os itens restantes de `../on-chain/security-checklist.md` (`SEC-01` a `SEC-12`) que ainda não tiverem sido fechados nas sprints anteriores. `SEC-11` passou de `pendente` para `parcialmente mitigado` — processo de rotação documentado (`../on-chain/runbooks/rotacao-trusted-issuer.md`); custódia real ainda depende da contratação do provedor de KYC (`PENDENCIAS.md`).
...
- [x] Documentar escopo e materiais de apoio para o auditor (`../on-chain/audit-package.md`, além de `on-chain/features/*/contracts/*.md`).
