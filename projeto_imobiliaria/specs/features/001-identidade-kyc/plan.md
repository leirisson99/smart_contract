...
| Backend da plataforma | Off-chain (feature 005) | Orquestra o fluxo, guarda PII em banco próprio (LGPD), aciona o provedor |

## Fluxo ponta a ponta
1. Investidor preenche cadastro na plataforma (off-chain) → RF-01.
2. Backend envia documentos ao provedor de KYC (off-chain).
3. Provedor aprova e assina uma claim `KYC_APPROVED` para a carteira custodial do investidor (criada automaticamente, ver feature 005).
...
## Dependências
Nenhuma — esta é a feature-base da qual `002`, `003` e `004` dependem para checagem de compliance.
