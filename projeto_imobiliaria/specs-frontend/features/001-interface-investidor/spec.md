...
Origem: pitch slide 4 (jornada completa do investidor) e slide 8 (painel do gestor). Contraparte de UI de [`specs-backend/features/001-plataforma-investidor`](../../../specs-backend/features/001-plataforma-investidor/spec.md) — o frontend nunca fala diretamente com os contratos on-chain; toda ação passa pelo backend (ver `integration.md` desta feature).

## Objetivo
Entregar a experiência visual e interativa que torna as features on-chain (`specs/features/001` a `004`) acessíveis a um investidor sem conhecimento técnico de blockchain, conforme prometido no pitch ("sem conhecimento técnico", slide 4).
...
- Qualquer validação de negócio que decida se uma ação é permitida — isso é sempre responsabilidade do backend/contrato (`specs-backend/features/001-plataforma-investidor`); a interface só reflete o resultado.
